...
class WorkerApiClient(_AuthenticatedClient):
...
class PoolApiClient(_AuthenticatedClient):
    ...
    def report_unhealthy(self, *, reason: str) -> None:
        response = self._request_json(
            "/pool/report_unhealthy",
            method="POST",
            payload=ReportPoolUnhealthyRequest(reason=reason).model_dump(mode="json"),
        )
        OkResponse.model_validate(response)
...
def create_manager_api_app(manager: Manager, *, auth_token: str) -> FastAPI:
    ...
    @pool_router.post("/report_unhealthy", response_model=OkResponse)
    def report_unhealthy(request: ReportPoolUnhealthyRequest) -> OkResponse:
        manager.report_pool_unhealthy(request.reason)
        return OkResponse()
