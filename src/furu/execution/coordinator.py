...
class ExecutionCoordinator:
    ...
    def run[ObjsT: Sequence[Furu]](
        ...
        *,
        ...
        (bind_host,) = {backend.coordinator_listen_host for backend in worker_backends}

        from furu.execution.server import execution_coordinator_server

        with coordinator.log_context():
            logger.info(
                "starting furu execution coordinator: executor_id=%s executor_dir=%s ready=%d blocked=%d",
    ...
    def job_result(self, lease_id: str, request: JobResultRequest) -> None:
        with self.log_context(), self.lock:
            ...
            logger.info(
                "execution coordinator progress: completed=%d/%d failed=%d running=%d ready=%d blocked=%d",
                len(self.completed),
    ...
    def _maybe_finish_locked(self) -> None:
        ...
        if terminal_failed or self.blocked:
            ...
            logger.error(
                "furu execution coordinator finished with error: %s", self.finish_error
            )
        else:
