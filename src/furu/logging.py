...
@contextmanager
def _scoped_log_files(log_paths: tuple[Path, ...] | list[Path]) -> Iterator[None]:
    normalized_paths = tuple(dict.fromkeys(log_paths))
    token = _CURRENT_LOG_PATHS.set(normalized_paths)
    try:
        yield
    finally:
