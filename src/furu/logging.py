...
def log_extra(
    *,
    ...
    """Build a logging ``extra=`` payload for Furu's structured renderers."""
...
def _console_mode(stream: object) -> tuple[bool, bool]:
    force = os.environ.get("FORCE_COLOR") is not None
...
class _FuruConsoleFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        console_layout, color = _console_mode(self._stream)
        if not console_layout:
        ...
        prefix_parts = [
            _ansi(timestamp, _COLORS["time"] if color else None),
            _ansi(level_letter, f"{level_color}{_BOLD}" if color else None),
        ]
        prefix_plain = f"{timestamp} {level_letter}"
        if component:
            prefix_parts.append(
                _ansi(component, _COLORS["component"] if color else None)
            )
        ...
        if record.exc_info:
            rendered += "\n" + self._format_console_exception(
                record.exc_info, color=color
            )
        return rendered
                ...
                if first_visual_row:
                    if caller:
                        ...
                        rows.append(
                            ...
                            + colored
                            + (" " * pad)
                            + _ansi(caller, _COLORS["caller"] if color else None)
                        )
                    else:
    ...
    def _format_console_exception(self, exc_info: Any, *, color: bool) -> str:
        lines = self.formatException(exc_info).splitlines()
        if not lines:
            return ""
        formatted: list[str] = []
        for line in lines[:-1]:
            formatted.append(_ansi(f"  | {line}", _DIM if color else None))
        formatted.append(_ansi(f"  | {lines[-1]}", _COLORS["error"] if color else None))
        return "\n".join(formatted)
...
def log_component(component: str | None) -> Iterator[None]:
    """Set the component label attached to log records in this context."""
