...
def _store_result[T](
    ...
    *,
    ...
    tmp_dir = _nfs_safe_unique_name(obj._result_dir, name="tmp")
    try:
        save_result_bundle(result, tmp_dir, obj._result_config())

        if not has_lock():
            raise LockLostError(
                f"lost lock at {obj._lock_path} after writing temporary result bundle"
            )

        if obj._result_dir.exists():
            shutil.rmtree(obj._result_dir)
        tmp_dir.rename(obj._result_dir)

        loaded = load_result_bundle(obj._result_dir, obj._result_config())
        ...
        if obj._result_dir.exists():
            shutil.rmtree(obj._result_dir, ignore_errors=True)
        raise
    ...
    *,
    ...
    for obj in unique:
        if obj._result_manifest_path.exists():
            obj.logger.info(
                "cache hit for %s at %s", obj._log_label, obj._result_manifest_path
            )
