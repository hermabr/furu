...
from furu.results import ResultConfig, dump_result_bundle, load_result_bundle
...
def _store_result[T](
    ...
    *,
    ...
    try:
        dump_result_bundle(result, tmp_result_dir, result_config)
        ...
        return cast(T, load_result_bundle(obj._result_dir, result_config))
    finally:
...
def load_or_create[T](
    ...
    *,
    ...
    result_configs_by_dir = {obj.data_dir: obj._result_config() for obj in unique}
    ...
    for obj in unique:
        if obj._result_manifest_path.exists():
            ...
            results_by_dir[obj.data_dir] = cast(
                ...
                load_result_bundle(
                    obj._result_dir,
                    result_configs_by_dir[obj.data_dir],
                ),
            )
        else:
            obj._internal_furu_dir.mkdir(parents=True, exist_ok=True)
    ...
    with lock_ctx as maybe_has_lock:
        ...
        for group in grouped.values():
            _execute_group(
                group,
                has_lock=has_lock,
                result_configs_by_dir=result_configs_by_dir,
...
def _execute_group[T](
    ...
    *,
    has_lock: HasLock,
    result_configs_by_dir: dict[Path, ResultConfig],
    results_by_dir: dict[Path, T],
) -> None:
        ...
        try:
            ...
            for obj, result in zip(group, results, strict=True):
                loaded_result = _store_result(
                    obj,
                    result,
                    result_config=result_configs_by_dir[obj.data_dir],
