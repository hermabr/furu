...
class SlurmWorkerPool:
    ...
    def _scale_once(self) -> dict[str, str]:
        # Lease recovery for vanished workers is not this pool's job: the
        # execution coordinator releases a worker's leases the moment its
        # WebSocket connection drops. This poll only tracks Slurm job ids so
        # scaling can submit replacements and surface failed jobs.
        active_job_ids = self._active_job_ids()
