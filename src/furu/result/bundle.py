...
class _DumpState:
    ...
    dumped_refs: list[_DumpedRef] = dataclasses.field(default_factory=list)


def _encode_metadata(value: object, data_dir: Path) -> JsonValue:
    if isinstance(value, Path):
        resolved = value.resolve()
        if not resolved.is_relative_to(data_dir.resolve()):
            raise ValueError(f"codec metadata path escapes data dir: {value}")
        return {_PATH_MARKER: resolved.relative_to(data_dir.resolve()).as_posix()}
    if isinstance(value, dict):
        return {str(key): _encode_metadata(child, data_dir) for key, child in value.items()}
    if isinstance(value, (list, tuple)):
        return [_encode_metadata(item, data_dir) for item in value]
    return cast(JsonValue, value)


def _decode_metadata(value: JsonValue, data_dir: Path) -> object:
    if isinstance(value, dict) and set(value) == {_PATH_MARKER}:
        rel = Path(cast(str, value[_PATH_MARKER]))
        if rel.is_absolute():
            raise ValueError(f"codec metadata path must be relative: {rel}")
        resolved = (data_dir.resolve() / rel).resolve()
        if not resolved.is_relative_to(data_dir.resolve()):
            raise ValueError(f"codec metadata path escapes data dir: {rel}")
        return resolved
    if isinstance(value, dict):
        return {key: _decode_metadata(child, data_dir) for key, child in value.items()}
    if isinstance(value, list):
        return [_decode_metadata(item, data_dir) for item in value]
    return value
...
def _dump_value(
    ...
    *,
    ...
    if isinstance(value, Ref):
        wrapper, artifact_rel, metadata = _dump_artifact(
            value.load(),
            codec=value._codec,
        ...
        dump_state.dumped_refs.append(
            _DumpedRef(value, artifact_rel, value._codec, metadata)
        )
        return wrapper

    if (codec := _annotated_codec(declared_type)) is not None:
    ...
    *,
    ...
    wrapper = {
        WRAPPER_KEY: {
            ...
            "metadata": _encode_metadata(dict(metadata), dump_state.data_dir),
        }
    }
    return wrapper, artifact_rel, metadata
    ...
    *,
    ...
    match kind:
        case "artifact":
            ...
            metadata = cast(Any, _decode_metadata(body["metadata"], data_dir))
            if get_origin(strip_annotated(declared_type)) is Ref:
                return Ref._from_storage(
                    artifact_dir=artifact_dir, codec=codec, metadata=metadata
                )
...
def load_result_bundle(
    bundle_dir: Path, *, data_dir: Path, declared_type: object = Any
) -> object:
