...
def migrate(obj: Furu[Any]) -> bool:
    ...
    pending: list[tuple[Migration, tuple[Migration, ...], frozenset[_Node]]] = [
        (migration, (), frozenset({target_node}))
        for migration in reversed(by_target.get(target_node, ()))
    ]
    while pending:
        migration, suffix, visited = pending.pop()
        old_node = (migration.old_fully_qualified_name, migration.old_schema_hash)
        if old_node in visited:
            continue

        migration_path = (migration, *suffix)
        ...
        next_visited = visited | {old_node}
        pending.extend(
            (next_migration, migration_path, next_visited)
            for next_migration in reversed(by_target.get(old_node, ()))
        )
    return False
