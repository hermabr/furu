...
class Added:
    ...
    ``default`` is the value every old run behaved as, or a function of one old
    run's stored fields (as ``Rewrite`` sees them) computing it per run, e.g.
    ``Added("model", default=lambda f: ModelConfig(width=f["width"]))``.
    Classes are values (``type``-valued fields), never called.
    """
    ...
    @property
    def derive(self) -> Callable[[Mapping[str, JsonValue]], Any] | None:
            ...
            case Added(field=field) as step:
                ...
                if not step.breaking and step.default is _NO_DEFAULT:
                    raise TypeError(
                        ...
                        "pinned independently of the field's own default, or a "
                        "function of the old run's stored fields computing it), "
                        "or breaking=True to discard the old results"
                    )
