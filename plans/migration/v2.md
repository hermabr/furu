...
Naming: user wants it called `schema_key` — do that. (Avoid `schema_version` to prevent confusion with existing `furu_package_version`.)
...
- `FuruMetadata` currently includes `furu_package_version` (package version), so avoid naming it `schema_version` to prevent confusion.
