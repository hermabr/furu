...
from furu.execution.coordinator import (
    FailedJob,
...
def _new_local_pool(
    *,
    server_url: str = "http://coordinator.test",
...
class CoordinatorLeaf(Furu[int]):
    value: int
...
class FlakyCoordinatorLeaf(Furu[int]):
    value: int
...
class CoordinatorParent(Furu[int]):
    child: CoordinatorLeaf

    def create(self) -> int:
        return self.child.create() + 1


class CoordinatorLazyParent(Furu[int]):
    value: int

    def create(self) -> int:
        return CoordinatorLeaf(value=self.value).create() + 1


def test_execution_coordinator_init_partitions_ready_and_blocked() -> None:
    leaf = CoordinatorLeaf(value=1)
    parent = CoordinatorParent(child=leaf)
...
def test_execution_coordinator_executor_id_is_stable_hash_of_root_object_tuple() -> (
    ...
    left = CoordinatorLeaf(value=1)
    right = CoordinatorLeaf(value=2)

    coordinator = _new_coordinator([left, right])

    assert len(coordinator.executor_id) == 32
    assert int(coordinator.executor_id, 16) >= 0
    assert _new_coordinator([left, right]).executor_id == coordinator.executor_id
    assert _new_coordinator([right, left]).executor_id != coordinator.executor_id
    assert (
...
def test_execution_coordinator_max_retries_per_object_defaults_to_config() -> None:
    coordinator = _new_coordinator([CoordinatorLeaf(value=1)])
...
def test_execution_coordinator_job_result_blocked_discovers_lazy_dependency_and_reruns_parent() -> (
    ...
    parent = CoordinatorLazyParent(value=2)
    dependency = CoordinatorLeaf(value=2)
    coordinator = _new_coordinator([parent])
...
def test_execution_coordinator_job_result_blocked_discovers_multiple_lazy_dependencies_together() -> (
    ...
    parent = CoordinatorLazyParent(value=2)
    dependencies = [CoordinatorLeaf(value=2), CoordinatorLeaf(value=3)]
    coordinator = _new_coordinator([parent])
...
def test_execution_coordinator_job_result_failed_finishes_with_error() -> None:
    leaf = CoordinatorLeaf(value=1)
    coordinator = _new_coordinator([leaf], max_retries_per_object=0)
    job = coordinator.lease_job(resources=ANY_RESOURCES)
    assert isinstance(job, Job)
...
def test_execution_coordinator_job_result_failed_retries_before_finishing() -> None:
    leaf = CoordinatorLeaf(value=1)
    coordinator = _new_coordinator([leaf], max_retries_per_object=2)
...
def test_execution_coordinator_job_result_failed_retry_can_later_complete() -> None:
    leaf = CoordinatorLeaf(value=1)
    coordinator = _new_coordinator([leaf], max_retries_per_object=1)
...
def test_execution_coordinator_run_retries_failed_worker_result() -> None:
    FlakyCoordinatorLeaf.attempts_by_value.clear()
    value = uuid4().int
    leaf = FlakyCoordinatorLeaf(value=value)
    objs = [leaf]
    ...
    assert FlakyCoordinatorLeaf.attempts_by_value == {value: 2}
...
def test_count_satisfiable_jobs_caps_at_max_workers_and_filters_by_requirements() -> (
    ...
    coordinator = _new_coordinator(
        [CoordinatorLeaf(value=1), CoordinatorLeaf(value=2), GpuLeaf(value=3)]
    )
...
def test_lease_job_filters_by_worker_resources() -> None:
    ...
    coordinator = _new_coordinator([cpu_leaf, gpu_leaf])
...
def test_execution_coordinator_run_passes_executor_dir_to_worker_backend() -> None:
    class RecordingBackend:
        coordinator_listen_host = "127.0.0.1"
    ...
    leaf = CoordinatorLeaf(value=12)
    expected_executor_dir = _new_coordinator([leaf]).executor_dir
    backend = RecordingBackend()
        ...
        def start_pool(
            ...
            *,
            ...
            raise AssertionError(
                "execution coordinator started workers with no runnable objects"
            )

    leaf = CoordinatorLeaf(value=15)
    leaf.create()
    objs = [leaf]
    coordinator = _new_coordinator(objs)
...
def test_execution_coordinator_run_uses_worker_backend_coordinator_listen_host() -> (
    None
):
    class RecordingPool:
    ...
    ExecutionCoordinator.run(
        [CoordinatorLeaf(value=15)], worker_backends=(backend,), port=0
    )
...
def test_execution_coordinator_server_exposes_bound_host_and_port() -> None:
    coordinator = _new_coordinator([CoordinatorLeaf(value=12)])
...
def test_execution_coordinator_run_requires_explicit_worker_backends() -> None:
    with pytest.raises(TypeError, match="worker_backends"):
        ExecutionCoordinator.run([CoordinatorLeaf(value=12)])  # ty: ignore[missing-argument]


def test_execution_coordinator_run_rejects_empty_worker_backends() -> None:
    with pytest.raises(ValueError, match="not enough values to unpack"):
        ExecutionCoordinator.run([CoordinatorLeaf(value=12)], worker_backends=())


def test_execution_coordinator_run_rejects_conflicting_coordinator_listen_host() -> (
    None
):
    with pytest.raises(ValueError, match="too many values to unpack"):
        ExecutionCoordinator.run(
            [CoordinatorLeaf(value=12)],
            worker_backends=(
                LocalThreadWorkerBackend(coordinator_listen_host="127.0.0.1"),
                LocalThreadWorkerBackend(coordinator_listen_host="0.0.0.0"),
            ),
        )
...
def test_execution_coordinator_api_rejects_missing_auth_token() -> None:
    app = create_execution_coordinator_api_app(
        _new_coordinator([CoordinatorLeaf(value=1)]), auth_token="secret"
    )
