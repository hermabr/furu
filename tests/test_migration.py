...
from furu.layout import _internal_furu_dir_in, _result_manifest_path_in
...
def test_migrate_with_matching_old_artifact_writes_result_link() -> None:
    ...
    link_path = _internal_furu_dir_in(new.data_dir) / "result-link.json"
...
def test_migrate_returns_false_when_already_has_direct_result() -> None:
    ...
    assert not (_internal_furu_dir_in(new.data_dir) / "result-link.json").exists()
...
def test_multi_hop_migration_points_directly_at_ultimate_source() -> None:
    ...
    link = json.loads(
        (_internal_furu_dir_in(final.data_dir) / "result-link.json").read_text()
    )
    assert link["source"]["data_dir"] == str(old.data_dir)
