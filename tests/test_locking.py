...
def test_stale_break_refuses_manifest_missing_contested_path(tmp_path: Path) -> None:
    lock_path = tmp_path / "broken.lock"
    claim_path = tmp_path / "claim.lock"
    lock_path.write_text(
        json.dumps(
            {
                "version": 1,
                "claim_path": str(claim_path.resolve()),
                "lock_paths": [str((tmp_path / "other.lock").resolve())],
            }
        ),
        encoding="utf-8",
    )
    ...
    with pytest.raises(LockAcquireError, match="contested lock path"):
        with lock(
            lock_path,
            lifetime_s=SHORT_LIFETIME_S,
            heartbeat_interval_s=SHORT_HEARTBEAT_INTERVAL_S,
