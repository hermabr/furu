...
from furu.config import Config, _FuruDirectories, _set_config
...
def _worker(data_dir: str, start_evt, out_q) -> None:
    _set_config(Config(directories=_directories_for_data(data_dir)))
    obj = SlowProbe(key=1)
...
def test_two_processes_competing_for_same_furu_object(tmp_path):
    ...
    assert (
        load_result_bundle(
            bundle_dir, data_dir=data_dir_in(bundle_dir.parent), declared_type=int
        )
        == 42
    )
