...
from furu import Furu, ResultConfig, load_or_create
...
def test_two_processes_competing_for_same_furu_object(tmp_path):
    ...
    result_paths = list(data_dir.glob("**/result/manifest.json"))
    assert len(result_paths) == 1
    assert load_result_bundle(result_paths[0].parent, ResultConfig.default()) == 42
