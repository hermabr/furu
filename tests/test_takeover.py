...
# ---------------------------------------------------------------------------
# Takeover handshake (old-coordinator side)
...
def test_takeover_handshake_surrenders_claimed_pools(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with _old_run_with_pool(tmp_path, monkeypatch) as (
        ...
        with _takeover_connection(server) as connection:
            connection.send(
                PoolsRequest(successor_executor_id=NEW_EXECUTOR_ID).model_dump_json()
            )
            inventory = PoolsResponse.model_validate_json(connection.recv(timeout=5))
            assert inventory.executor_id == OLD_EXECUTOR_ID
            (pool_inventory,) = inventory.pools
            assert pool_inventory.pool_id == pool.pool_id
            assert [(job.job_id, job.state) for job in pool_inventory.jobs] == [
                ("100_0", "RUNNING"),
                ("100_1", "PENDING"),
            ]

            connection.send(ClaimRequest(adopt=[pool.pool_id]).model_dump_json())
            response = ClaimResponse.model_validate_json(connection.recv(timeout=5))
            assert response.cancelled == []
            (adopted,) = response.adopted
            assert [job.job_id for job in adopted.jobs] == ["100_0", "100_1"]
...
def test_takeover_handshake_rejects_unknown_pool_ids(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with _old_run_with_pool(tmp_path, monkeypatch) as (coordinator, server, pool, _):
        with _takeover_connection(server) as connection:
            ...
            connection.send(ClaimRequest(adopt=["deadbeef"]).model_dump_json())
...
def test_successor_drop_before_claim_keeps_all_pools(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    with _old_run_with_pool(tmp_path, monkeypatch) as (coordinator, server, pool, _):
...
def test_second_takeover_connection_is_rejected_as_busy(
    ...
    with (
        ...
        _takeover_connection(server) as first,
    ):
        first.send(
            PoolsRequest(successor_executor_id=NEW_EXECUTOR_ID).model_dump_json()
        )
        first.recv(timeout=5)  # the handshake is now in progress
...
def test_perform_takeover_errors_when_no_pool_matches(
    ...
    with _old_run_with_pool(tmp_path, monkeypatch) as (old_coordinator, _, pool, _):
        ...
        # The old run is unaffected: nothing was claimed.
