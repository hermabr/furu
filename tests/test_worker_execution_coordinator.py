...
def test_execution_coordinator_job_result_completed_moves_dependents_to_ready() -> None:
    ...
    assert running_job.worker == "test-worker"
...
def test_execution_coordinator_job_result_blocked_discovers_lazy_dependency_and_reruns_parent() -> (
    ...
    parent_job = _lease_job(coordinator)
    ...
    dependency_job = _lease_job(coordinator)
...
def test_execution_coordinator_uses_new_lease_when_blocked_job_is_released() -> None:
    ...
    first_parent_job = _lease_job(coordinator)
    ...
    second_parent_job = _lease_job(coordinator)
...
def test_execution_coordinator_job_result_failed_retries_before_finishing(
    ...
    first_job = _lease_job(coordinator)
    ...
    second_job = _lease_job(coordinator)
    ...
    third_job = _lease_job(coordinator)
...
def test_execution_coordinator_job_result_failed_retry_can_later_complete() -> None:
    ...
    retry_job = _lease_job(coordinator)
...
def test_worker_lost_lease_stops_counting_against_worker_cap() -> None:
    limited = [LimitedExecutionCoordinatorLeaf(value=value) for value in range(3)]
    ...
    coordinator.worker_lost("slurm-worker-100a0")
...
def test_lease_job_filters_by_worker_memory_gib() -> None:
    ...
    assert _lease_job(coordinator, resources=ResourceRequest(memory_gib=7)) == "wait"

    memory_job = _lease_job(coordinator, resources=ResourceRequest(memory_gib=8))
...
def test_worker_loop_exits_after_idle_timeout(
    ...
    assert test_client.workers == ["test-worker"]
