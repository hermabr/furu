...
from furu.execution.takeover import AdoptedPool
...
def test_slurm_worker_pool_stop_leaves_jobs_alone_after_surrender(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _disable_slurm_pool_scale_thread(monkeypatch)
...
def test_slurm_worker_pool_reports_takeover_inventory(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _disable_slurm_pool_scale_thread(monkeypatch)
    ...
    inventory = pool.takeover_inventory()

    assert inventory.pool_id == pool.pool_id
    assert inventory.fingerprint == backend.fingerprint()
    assert inventory.endpoint_file == pool._endpoint_file
    assert [(job.job_id, job.state) for job in inventory.jobs] == [
        ("100_0", "RUNNING"),
        ("100_1", "PENDING"),
    ]
