...
if TYPE_CHECKING:
    LetterCountSpec: TypeAlias = Spec[int]
else:
    LetterCountSpec = type(letter_count("", ""))
...
class ExplicitCachedDependencyParent(Spec[str]):
    name: str
...
def test_function_creates_furu_object_from_function_signature():
    ...
    assert obj.create() == 3
    assert letter_count("banana", "a") == obj
...
def test_function_supports_defaults_and_artifact_round_trip():
    ...
    assert getattr(obj, "letter") == "a"
    assert letter_count_with_default("banana") == obj
    assert obj.create() == 3
...
def test_function_defaults_unannotated_parameters_to_any():
    ...
    assert letter_count_with_untyped_source("banana", "a") == obj
    assert obj.create() == 3
...
def test_function_supports_parenthesized_decorator():
    ...
    assert letter_count_with_parentheses("banana", "n") == obj
    assert obj.create() == 2
    assert obj._fully_qualified_name == "test_core.letter_count_with_parentheses"
    assert _from_json(obj._artifact_data) == obj


def test_function_hides_generated_spec_type_attrs():
    obj = letter_count("banana", "a")

    assert not hasattr(letter_count, "furu_type")
    assert not hasattr(letter_count, "spec")
    assert isinstance(obj, Spec)
    assert getattr(obj, "source") == "banana"
    assert getattr(obj, "letter") == "a"
    assert obj.create() == 3
...
def test_dependency_rejects_uncached_variant() -> None:
    with pytest.raises(TypeError, match="cached"):

        class InvalidUncachedDependency(Spec[str]):
            @furu.dependency(cached=False)  # ty: ignore[no-matching-overload]
            def child(self) -> Node:
                return Node(name="uncached")
...
def test_top_level_create_accepts_single_spec() -> None:
    node = Node(name="top-level-single")

    assert furu.create(node) == "Node(top-level-single)"


def test_top_level_create_accepts_list() -> None:
    nodes = [Node(name="top-level-a"), Node(name="top-level-b")]

    assert furu.create(nodes) == ["Node(top-level-a)", "Node(top-level-b)"]
...
def test_reserved_spec_field_names_are_rejected() -> None:
    with pytest.raises(TypeError) as exc_info:

        class ReservedMetadataName(Spec[int]):
            metadata: str
