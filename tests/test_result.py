...
def test_json_only_bundle_round_trips() -> None:
    ...
    assert not (result_dir_in(obj.data_dir) / "artifacts").exists()

    manifest = json.loads(result_manifest_path_in(obj.data_dir).read_text())
...
def test_scalar_root_manifest_is_just_the_value() -> None:
    ...
    text = result_manifest_path_in(obj.data_dir).read_text()
...
def test_annotated_codec_selects_external_artifact() -> None:
    ...
    assert (result_dir_in(obj.data_dir) / "artifacts" / "weights" / "data.npy").exists()
...
def test_polars_dataframe_round_trips() -> None:
    ...
    assert (
        result_dir_in(obj.data_dir) / "artifacts" / "frame" / "data.parquet"
    ).exists()
    assert isinstance(loaded, dict)
    frame = loaded["frame"]
...
def test_nested_numpy_paths_use_list_length_padded_indexes() -> None:
    ...
    layers_dir = result_dir_in(obj.data_dir) / "artifacts" / "layers"
    for i in range(10):
