...
class _DataDirPathCodec(Codec[_DataDirPathValue]):
    ...
    def save(
        ...
        artifact_dir.joinpath("path.txt").write_text("ok", encoding="utf-8")
...
def test_ref_mixed_result_round_trips_eager_and_lazy_fields() -> None:
    obj = RefArrayResult()
    loaded = obj.create()

    assert isinstance(loaded, RefArrayOutput)
    ...
    assert isinstance(loaded.ref_weights, furu.Ref)
    assert not loaded.ref_weights.is_loaded
    assert np.array_equal(loaded.ref_weights.load(), np.arange(5))

    loaded_again = obj.load_existing()

    assert isinstance(loaded_again.weights, np.ndarray)
    assert isinstance(loaded_again.ref_weights, furu.Ref)
    assert not loaded_again.ref_weights.is_loaded
    assert np.array_equal(loaded_again.ref_weights.load(), np.arange(5))


def test_ref_without_resolvable_codec_fails_at_call_site() -> None:
    with pytest.raises(ValueError, match="plain eager field|explicit codec"):
        furu.ref([1, 2, 3])


def test_ref_conflicts_with_annotated_codec() -> None:
...
@dataclass(frozen=True)
class _SwitchRef:
    weights: furu.Ref[Any]


def test_ref_and_eager_codec_field_use_identical_artifact_wrapper(
    tmp_path: Path,
) -> None:
    ...
    _save_result_bundle(
        _SwitchEager(weights=value),
        eager_dir,
        declared_type=_SwitchEager,
        result_codecs=(),
    )
    _save_result_bundle(
        _SwitchRef(weights=furu.ref(value)),
        ref_dir,
        declared_type=_SwitchRef,
        result_codecs=(),
    )
    ...
    assert (
        eager_manifest["$furu"]["|fields"]["weights"]
        == ref_manifest["$furu"]["|fields"]["weights"]
    )
...
def test_data_dir_result_codec_round_trips_shared_data_dir_path() -> None:
    ...
    assert (artifacts_dir / "first" / "path.txt").read_text() == "ok"
    assert (artifacts_dir / "second" / "path.txt").read_text() == "ok"
    assert not (artifacts_dir / "first" / "data.zarr").exists()
    manifest = json.loads(result_manifest_path_in(obj._base_dir).read_text())
    assert manifest["first"]["$furu"]["metadata"]["path"] == {
        "$furu": {"|kind": "data_path", "value": "data.zarr"}
    }
...
def test_data_dir_result_codec_rejects_load_path_outside_data_dir(
    ...
    manifest["$furu"]["metadata"]["path"]["$furu"]["value"] = "../outside"
...
def test_data_dir_result_codec_rejects_save_path_outside_data_dir(
    tmp_path: Path,
) -> None:
    with pytest.raises(ValueError, match="inside data dir"):
        _save_result_bundle(
            _DataDirPathValue(tmp_path / "outside"),
            tmp_path / "bundle",
...
def test_root_ref_defers_cache_read_and_memoizes(
    ...
    loaded = load_result_bundle_impl(
        bundle_dir,
        data_dir=data_dir_in(bundle_dir.parent),
...
def test_nested_ref_exposes_persisted_path(tmp_path: Path) -> None:
    ...
    assert ref.path == bundle_dir / "artifacts" / "outer" / "inner"
    assert ref.path.joinpath("value.txt").exists()
    assert ref.load().value == 12


def test_nested_ref_round_trips_inside_supported_structures(
    tmp_path: Path,
) -> None:
    bundle_dir = tmp_path / "bundle"
    result_codecs = (_CountingCodec,)
    ...
    loaded = load_result_bundle_impl(
        bundle_dir,
        data_dir=data_dir_in(bundle_dir.parent),
        declared_type=dict[str, list[furu.Ref[_CountingValue]]],
    )
    ...
    first = cast(furu.Ref[_CountingValue], items[0])
    second = cast(furu.Ref[_CountingValue], items[1])
    assert isinstance(first, furu.Ref)
    assert isinstance(second, furu.Ref)
...
def test_load_result_bundle_rejects_artifact_path_escape(tmp_path: Path) -> None:
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    (bundle_dir / "artifacts").mkdir()
    (bundle_dir / "manifest.json").write_text(
...
def test_load_result_bundle_rejects_artifact_without_directory(
    tmp_path: Path,
) -> None:
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    ...
    with pytest.raises(ValueError, match="artifact directory missing"):
        load_result_bundle(bundle_dir)
